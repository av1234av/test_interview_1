...
class AssetRiskReport(object):
    ...
    def _calc_position(self, trade):
        pos = self._position.get(trade.symbol, {'Position': 0, 'TradeVol': 0, 'MktVal':0.0})
        pos['Position'] += trade.qty * (1 if trade.side == 'BUY' else -1)
        pos['TradeVol'] += trade.qty
    ...
    def generate_report(self):
        ...
        for item in sorted(self._position.items(), key=lambda x: x[1]['TradeVol'], reverse=True)[:20]:
            print item[0], item[1]['TradeVol']

if __name__ == '__main__':
    path='C:\\Users\\amit_\\PycharmProjects\\asset_risk\\Data'
